...
from google.cloud.functions import *
...
# update user details
@app.route('/Users/<userEmail>', methods=['PATCH'])
...
#  Create new notification in firestore
    notificationRef.document(notificationid).set(notification)
    return jsonify({'user': 'notification created'}), 201



# Get all notifcaton details 
...
def listenNewPost(event, context):
    query = postRef.where('PostID', '==', event['PostID']).where('newPost', '==', True)
    snapshot = query.listen()

    for change in snapshot:
        posts = change.document.to_dict()
        postOwner = posts['userName']
        print('user')
        if posts['newPost']:
            sendEmail(postOwner)
            change.document.update({'newPost':False})
